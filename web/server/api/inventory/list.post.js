...
// Not defineEntityHandler: InventoryListRequest.skuId/warehouseId are
...
export default defineEventHandler(async (event) => {
  ...
  const { skuId, warehouseId, ...filter } = body.filter || {}
  const client = getServiceClient('inventory.proto', 'crm.grpc.inventory.v1.InventoryService')
  try {
    const response = await grpcCall(
      client,
      'List',
      { skuId, warehouseId, filter, pagination: body.pagination },
      { token: session.token },
    )
