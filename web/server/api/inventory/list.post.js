...
// Not defineEntityHandler: InventoryListRequest.productId/warehouseId are
...
export default defineEventHandler(async (event) => {
  ...
  const { productId, warehouseId, ...filter } = body.filter || {}
  const client = getServiceClient('inventory.proto', 'crm.grpc.inventory.v1.InventoryService')
  try {
    const response = await grpcCall(
      client,
      'List',
      { productId, warehouseId, filter, pagination: body.pagination },
      { token: session.token },
    )
