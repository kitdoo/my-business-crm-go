...
// Shared by both katalog/index.vue (categorySlug=null, "all") and
// katalog/kategorija/[category].vue (categorySlug from the route) — see
// nuxt.config.js's i18n.pages for why the category page lives at its own
// path instead of an optional segment on this same file: katalog/[sku].vue
// already claims the single-segment route space under /katalog, so a
// category slug there would collide with product permalinks.
...
// Categories are resolved eagerly (not lazy) here: on a direct hit to a
// /katalog/kategorija/:slug URL we need the real categoryId *before*
// deciding what to fetch for the product grid (and whether to 404) — there
// is no backend endpoint that takes a slug directly, since the slug is a
// frontend-only construct (see slugify.js) derived from the category name,
// not a stored field. The list itself is small, so blocking briefly on it
// (also on the plain /katalog page, for one shared code path) is cheap.
// useAsyncData never throws (errors land in categoriesFetchError instead),
// so a backend outage can't crash this into an unstyled 500 — it falls
// through to the same loadError state the product grid uses below instead
// of the 404 (see the guard on that check).
const { data: categoriesData, error: categoriesFetchError } = await useAsyncData('catalog-categories', () => listCategories())
const categories = computed(() => categoriesData.value?.items || [])
// Falls back to the raw id when the sr name slugifies to '' (e.g. a name
// written in a script slugify() doesn't transliterate) — without this, that
// category's URL carries an empty :category param, router.resolve() throws
// "Missing required param", @nuxtjs/i18n's localePath() swallows it and
// returns '', and navigateTo('') silently redirects home instead of to the
// category (this is the bug that shipped before this fallback existed).
function categorySlugOf(category) {
  return slugify(category.name?.values?.sr) || category.id
}

// null = "no category constraint" (the plain /katalog page); undefined =
// "a slug was given but doesn't match any known category" (404, below).
const activeCategory = computed(() => {
  if (!props.categorySlug) return null
  return categories.value.find((c) => categorySlugOf(c) === props.categorySlug)
})
// categoriesData is awaited above (not lazy), so — outages aside — this is
// a reliable 404, not a "hasn't loaded yet" false positive.
if (props.categorySlug && !categoriesFetchError.value && !activeCategory.value) {
  throw createError({ statusCode: 404, statusMessage: 'Category not found', fatal: true })
}

function categoryHref(id) {
  if (id === ALL_CATEGORIES) return localePath('/katalog')
  const category = categories.value.find((c) => c.id === id)
  return localePath({ name: 'katalog-kategorija-category', params: { category: categorySlugOf(category) } })
}
// Reselecting a category navigates to its own URL instead of just updating
// state in place — that's what makes each category a real, distinct,
// crawlable page rather than a client-side filter on one shared URL.
const activeCategoryId = computed({
  get: () => activeCategory.value?.id ?? ALL_CATEGORIES,
  set: (id) => navigateTo(categoryHref(id)),
})
...
// Category changes navigate to a different URL (see activeCategoryId's
// setter above) rather than updating state in place, so they're excluded
// here — this only reacts to the filters that stay query params on
// whichever /katalog... path is current.
watch([activeSort, activeInStock], () => {
  router.replace({
    query: {
      ...route.query,
...
// Covers both a direct hit changing categorySlug (SSR) and the client-side
// route reuse Vue Router does when only :category changes (component stays
// mounted, so the watch above never fires for it).
watch(() => activeCategory.value?.id, (id, prevId) => {
  if (id === prevId) return
  resetAndLoad()
})

const categoryName = computed(() => (activeCategory.value ? localizedText(activeCategory.value.name, locale.value) : null))
const pageTitle = computed(() => (categoryName.value ? `${categoryName.value} — ${t('seo.katalog.title')}` : t('seo.katalog.title')))
const pageDescription = computed(() => {
  if (!activeCategory.value) return t('seo.katalog.description')
  const description = localizedText(activeCategory.value.description, locale.value)
  return description || t('seo.katalog.description')
})
const introText = computed(() => (categoryName.value ? pageDescription.value : t('catalog.intro')))
...
<template>
  <div class="mx-auto max-w-7xl 2xl:max-w-[1800px] px-4 lg:px-8 py-12 lg:py-16">
    <h1 class="text-2xl lg:text-3xl font-bold uppercase tracking-wide mb-4">{{ categoryName || t('nav.katalog') }}</h1>
    <p class="text-black/70 leading-relaxed max-w-3xl mb-8">{{ introText }}</p>
