...
type Service interface {
	// Create returns errs.ErrProductNotFound if productId does not resolve
	// to an existing product, or errs.ErrProductPriceExists if the product
	// already has an active price.
	Create(ctx context.Context, in *entities.ProductPriceCreate) (*entities.ProductPrice, error)
	// Get looks up the current price for a product.
	Get(ctx context.Context, productID string) (*entities.ProductPrice, error)
