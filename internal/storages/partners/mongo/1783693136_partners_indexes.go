...
func init() {
	migrate.MustRegister(func(ctx context.Context, db *mongo.Database) error {
		...
		if err := coll.Indexes().DropOne(ctx, "idx_partners_deleted_at_created_at"); err != nil {
			return err
		}
		if err := coll.Indexes().DropOne(ctx, "idx_partners_deleted_at_name"); err != nil {
			return err
		}
		return coll.Indexes().DropOne(ctx, "idx_partners_phone_unique")
	})
}
