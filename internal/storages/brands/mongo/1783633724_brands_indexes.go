		...
		_, err := db.Collection(collectionName).Indexes().CreateMany(ctx, []mongo.IndexModel{
			{
				...
				Options: options.Index().
					SetName("idx_brands_deleted_at_created_at"),
			},
			{
				Keys: bson.D{{Key: FieldDeletedAt, Value: 1}, {Key: FieldNameSr, Value: 1}},
				Options: options.Index().
					SetName("idx_brands_deleted_at_name_sr"),
			},
		})
		return err
	}, func(ctx context.Context, db *mongo.Database) error {
		coll := db.Collection(collectionName)
		if err := coll.Indexes().DropOne(ctx, "idx_brands_deleted_at_created_at"); err != nil {
			return err
		}
		return coll.Indexes().DropOne(ctx, "idx_brands_deleted_at_name_sr")
	})
}
