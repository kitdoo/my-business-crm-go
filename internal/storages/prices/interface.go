...
// Storage persists the current price per product plus an append-only price
// history log. All reads against the current price filter out soft-deleted
// rows.
type Storage interface {
	// Insert returns errs.ErrProductPriceExists when productId already has
	// an active price.
	...
	// GetByProductID looks up the current price for a product.
	GetByProductID(ctx context.Context, productID string) (*entities.ProductPrice, error)
	...
	// GetHistory lists price history log entries for a product.
