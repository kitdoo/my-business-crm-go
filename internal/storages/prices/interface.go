...
// Storage persists the current price per SKU plus an append-only price
// history log. All reads against the current price filter out soft-deleted
// rows.
type Storage interface {
	// Insert returns errs.ErrProductPriceExists when skuId already has
	// an active price.
	...
	// GetHistory lists price history log entries for a SKU.
