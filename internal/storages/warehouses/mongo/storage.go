...
type model struct {
	...
	Status      entities.WarehouseStatus `bson:"status"`
...
var _ warehouses.Storage = (*Storage)(nil)

// Storage implements warehouses.Storage against MongoDB.
type Storage struct {
	db         *datamongo.Mongo
	collection *mongo.Collection
}

// New builds a Storage backed by db's "warehouses" collection.
...
func (s *Storage) Insert(ctx context.Context, w *entities.Warehouse) error {
	ctx, cancel := context.WithTimeout(ctx, datamongo.DefaultQueryTimeout)
	defer cancel()

	m := converter.Convert(w, &model{}, converter.WithHandleEmbeddedStructs(true))
	m.CursorId = bson.NewObjectID()
	doc, err := s.db.ConvertToNewDocument(ctx, m)
	if err != nil {
		return coreerrs.WrapOperation(err, "convert warehouse document")
	}
	if _, err := s.collection.InsertOne(ctx, doc); err != nil {
		return coreerrs.WrapOperation(err, "insert warehouse")
	}
	return nil
}

func (s *Storage) Get(ctx context.Context, id string) (*entities.Warehouse, error) {
	ctx, cancel := context.WithTimeout(ctx, datamongo.DefaultQueryTimeout)
	defer cancel()
	...
	if err := s.collection.FindOne(ctx, activeOnly(bson.M{FieldID: id})).Decode(&m); err != nil {
		...
		return nil, coreerrs.WrapOperation(err, "get warehouse")
	}
	return converter.Convert(&m, &entities.Warehouse{}), nil
}

func (s *Storage) Update(ctx context.Context, w *entities.Warehouse, oldEtag string) error {
	ctx, cancel := context.WithTimeout(ctx, datamongo.DefaultQueryTimeout)
	defer cancel()

	update, err := s.db.ConvertToUpdateDocument(ctx, converter.Convert(w, &model{}))
	if err != nil {
		return coreerrs.WrapOperation(err, "build update document")
	}
	filter := activeOnly(bson.M{FieldID: w.ID})
	if oldEtag != "" {
		filter[FieldEtag] = oldEtag
	}
	res, err := s.collection.UpdateOne(ctx, filter, update)
	if err != nil {
		return coreerrs.WrapOperation(err, "update warehouse")
	}
...
func (s *Storage) SoftDelete(ctx context.Context, in *entities.SoftDelete) error {
	...
	res, err := s.collection.UpdateOne(ctx, filter, update)
	if err != nil {
		return coreerrs.WrapOperation(err, "soft delete warehouse")
	}
	if res.MatchedCount == 0 {
		if in.Etag != "" {
			return errs.ErrStaleEntity
...
func (s *Storage) List(ctx context.Context, in *entities.WarehousesList) (*entities.List[entities.Warehouse], error) {
	ctx, cancel := context.WithTimeout(ctx, datamongo.DefaultQueryTimeout)
	defer cancel()
	...
	if err != nil {
		...
		return nil, coreerrs.WrapOperation(err, "list warehouses")
	}

	out := &entities.List[entities.Warehouse]{
		Items: coreslices.To(result.Items, func(m model) *entities.Warehouse {
			return converter.Convert(&m, &entities.Warehouse{})
		}),
		Total: result.Total,
	}
...
func listSortField(sort entities.WarehousesListSort) string {
	field := FieldCreatedAt
	if sort.Field == entities.WarehousesListSortFieldName {
		field = FieldNameSr
	}
