...
func init() {
	migrate.MustRegister(func(ctx context.Context, db *mongo.Database) error {
		_, err := db.Collection(collectionName).Indexes().CreateMany(ctx, []mongo.IndexModel{
			{
				Keys: bson.D{{Key: FieldSKUID, Value: 1}, {Key: FieldWarehouseID, Value: 1}, {Key: FieldCreatedAt, Value: -1}},
				Options: options.Index().
					SetName("idx_inventory_movements_sku_warehouse_created_at"),
			},
			{
		...
		if err := coll.Indexes().DropOne(ctx, "idx_inventory_movements_sku_warehouse_created_at"); err != nil {
			return err
		}
