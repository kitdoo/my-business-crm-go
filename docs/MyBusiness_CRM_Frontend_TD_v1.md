...
├─────────────────────────────────────────────────┬───────────────┤
│                                                   │               │
│                Основной контент                  │  RightDrawer  │
│      (Dashboard ИЛИ список сущности —             │  (тёмно-серый,│
│       карточки/таблица + кнопка «Создать»)         │  #333333):    │
│                                                   │  Dashboard    │
│                                                   │  Бренды       │
│                                                   │  Категории    │
│                                                   │  ...          │
│  Клик по строке / иконке ✏️ / кнопке «Создать» —   │  (плоский     │
│  открывает EditDrawer слева поверх контента        │  список, без  │
│  (см. 8.3), список остаётся видимым сбоку.         │  разделов)    │
└─────────────────────────────────────────────────┴───────────────┘
```
...
- На `< lg` — гамбургер-иконка слева от лого, открывающая правый drawer как overlay (см. п.6).

### 8.2 RightDrawer — плоское меню (постоянный на `≥ lg`, overlay на `< lg`)
...
export const NAV_ITEMS = [
  ...
  // остальное дописывается в RightDrawer.vue из listEntityConfigs() —
...
### 8.3 EditDrawer — создание/редактирование в выезжающем слева drawer

Клик по строке списка, по иконке ✏️ в колонке «Действия» (п.9.3) или по кнопке «Создать» открывает `USlideover` со стороны `left` поверх текущего контента (список остаётся виден и не теряет состояние/скролл/фильтры) — **не** переход на отдельную страницу `/[entity]/new` или `/[entity]/[id]`. Внутри — тот же generic `<EntityForm>` (п.9.3), в режиме `mode="drawer"`: вместо `router.push` после сохранения/удаления форма эмитит `saved`/`cancel`/`deleted`, хозяин (`EntityListPage`) закрывает drawer и обновляет таблицу на месте.

- Владелец состояния drawer (открыт/закрыт, id редактируемой записи) — сама `EntityListPage`, не глобальный layout-стор: drawer нужен только там, где есть список.
...
app/
  components/
    ...
    layout/         # TopBar, RightDrawer (плоское меню, см. п.8.2)
